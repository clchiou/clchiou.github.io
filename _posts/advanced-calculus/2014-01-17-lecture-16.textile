...
If $\mathcal{F}$ is a family of open sets covering $[a, b]$ ($\forall x \in [a, b], \exists V \in \mathcal{F}$ s.t. $x \in V$), then there are finitely many open sets $V_1, V_2, \dotsc, V_n \in \mathcal{F}$ s.t. $[a, b] \subset \bigcup_{i=1}^n V_i.$

To prove Heine-Borel Theorem, we have to go back to the topology of $\mathbb{R}.$

_Definition._ Given $A \subset \mathbb{R},$ $x$ is an _accumulation point_ of $A$ if all neighborhoods of $x$ contain infinitely many points of $A$ (or contain at least one point of $A$ that is not $x,$ or $\forall \delta > 0, (x - \delta, x + \delta) \cup A$ is infinite).
